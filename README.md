...
written list of "areas to explore next" generated by OpenAI from the transcript.
...
4. When the student clicks "I'm done — see my feedback", the summary page polls
   the server, which fetches the call transcript from BeyondPresence and sends
   it to OpenAI for a structured analysis.
5. The student sees what was covered, their apparent level, and concrete next
   steps to improve.
...
2. Copy the env example and fill in your keys:
   ...
   BEY_DEFAULT_AVATAR_ID=...      # see below
   OPENAI_API_KEY=...
   # Optional, defaults to gpt-4o-mini
   # OPENAI_MODEL=gpt-4o-mini
   ```

3. **Pick an avatar UUID** for `BEY_DEFAULT_AVATAR_ID`:

   - Easiest: open the [BeyondPresence dashboard](https://app.bey.chat) and pick
...
- Click **I'm done — see my feedback** (or use the avatar's end-call control)
  to land on the summary page, where the transcript is analyzed and you get a
  written list of next steps.
...
src/
  app/
    ...
    api/
      ...
      summary/[id]/route.ts          # GET: fetch transcript + OpenAI analysis
  lib/
...
- Tailwind CSS
- BeyondPresence managed agents + iframe embed
- OpenAI Chat Completions (`gpt-4o-mini` by default) for post-call analysis
